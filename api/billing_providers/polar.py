...
class PolarBillingProvider:
    name = _POLAR_PROVIDER

    async def create_checkout(self, checkout: CheckoutInput) -> CheckoutResult:
        return_url = _resolve_checkout_return_url(checkout.locale, checkout.return_path)
        metadata = {"user_id": checkout.user_id, "plan_code": checkout.plan_code}
        if checkout.city_id:
            metadata["city_id"] = checkout.city_id
        session = await create_polar_checkout_session(
            products=[_polar_product_id(checkout.plan_code)],
            success_url=return_url,
            return_url=return_url,
            external_customer_id=checkout.user_id,
            customer_email=checkout.identity["email"],
            customer_name=checkout.identity["name"],
            metadata=metadata,
    ...
    async def complete_payment(self, user_id: str, payment_id: str) -> dict:
        return {"ok": True, "provider": self.name, "payment_id": payment_id, "unlocked": False}
    ...
    async def restore(self, user_id: str) -> dict:
        ...
        return {"ok": True, "restored": restored, "provider": self.name}

    async def handle_webhook(self, request: Request) -> JSONResponse:
        return await polar_webhook(request)
...
def _polar_product_id(plan_code: str) -> str:
    mapping = {
        ...
        "city_report": _env("POLAR_PRODUCT_CITY_REPORT"),
    }
