...
test('restores visible country briefing markdown into formatted briefing data', () => {
  const markdown = briefingToMarkdown({
    documentId: 'NNAI-TH-20260523-abc123',
    issuedDate: '2026-05-23',
    preparedFor: 'Free Spirit',
  ...
  const restored = briefingFromMarkdown(markdown);

  assert.equal(restored?.cityKr, '방콕');
  assert.equal(restored?.cityName, 'Bangkok');
  assert.equal(restored?.quickFacts.visa, 'DTV');
