...
도시 specific 데이터 필수 (Risk Notes 5번에서 특히 중요):
- 기후: 월/평균 온도/특이 현상/외무부 안전공지 사례까지 구체적으로 명시.
  예: "치앙마이는 2-4월 burning season으로 PM2.5가 위험 수준(150-300 µg/m³)에 도달합니다."
  예: "마드리드는 7-8월 폭염기 40°C 이상이 빈번하며 한국 외교부 폭염 안전공지 발령 사례가 있습니다."
  예: "도쿄는 7-8월 35°C 이상 다습이 지속되며 일본 정부는 매년 수백 명의 열중증 사망 사례를 발표합니다."
- 일반론 ("도시별 우기와 건기로 구분됩니다") 금지. 도시 이름과 월/온도/사례 함께 명시.
- 정보가 불확실하면 그 항목은 작성 금지. 추정·일반론으로 대체 금지.
...
- references 배열의 num은 본문 마커와 1:1 일치해야 한다.

페르소나 정의:
- wanderer: 이동 지향, 무비자/비자런 중심, 장기 정착 정보 불필요.
- local: 관계와 커뮤니티 중심, 한인 사회 및 현지 편입 중심.
- planner: 합리성과 수치 중심, 세금 및 재정 정보 상세하게.
- free_spirit: 일상 만족 중심, 날씨/카페/스트레스 없는 삶 중심.
- pioneer: 정착 탐색 중심, 장기비자/영주권/사업자 등록 중심.

문체 규칙:
- 본문, items, table notes 내에서 가운뎃점(·) 사용 금지. 항목 나열은 쉼표(,) 또는 슬래시(/).
- 가운뎃점은 LLM 생성 문체 신호로 인식되어 사용자 신뢰도를 떨어뜨림.

출력은 단일 JSON 객체이며, 최상위 키는 "sections"와 "references" 두 가지다.
sections는 BriefingSection 배열, references는 BriefingReference 배열.
각 섹션과 서브섹션의 num/title은 입력 스켈레톤을 그대로 사용하고 body/items/table만 도시 데이터로 채운다.`;
